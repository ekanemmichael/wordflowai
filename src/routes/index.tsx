...
function OperatorConsole() {
  ...
  // ── Display a verse on the projector ──────────────────────────────────────
  ...
  // Pick best verse from a list and display it
  const autoDisplay = useCallback((refs: ResolvedVerse[]) => {
    const best =
  ...
  const handleInterim = useCallback((interim: string) => {
    ...
    interimDebounce.current = setTimeout(async () => {
      ...
      // Quick check: any explicit ref in interim before making fetch calls
  ...
  // ── Tier 2: client-side detection on final sentence (still instant) ────────
  const handleFinal = useCallback(async (text: string) => {
    const next = sermon ? sermon + " " + text : text;
    setSermon(next);

    // Check translation switch
    ...
    // Tier 3: AI detection (background, catches implied refs + quotations)
    ...
    aiDebounce.current = setTimeout(async () => {
      ...
      } catch { /* AI unavailable — regex already handled it */ }
    }, 500);
  }, [sermon, settings.translation, update, autoDisplay, aiDetect]);

  // Manual textarea: run client-side detection
  useEffect(() => {
      ...
      <main className="flex-1 grid lg:grid-cols-[1fr_400px] overflow-hidden">
        ...
        <div className="flex flex-col p-6 gap-4 border-r border-white/[0.07]">
          ...
          <p className="text-white/20 text-[11px] text-center">
            Open <span className="text-amber-400/60">/display</span> on the second screen and drag it to the projector
          </p>
        </div>
        ...
        <div className="flex flex-col overflow-y-auto">
          ...
          <div className="flex flex-col items-center gap-4 px-6 py-8 border-b border-white/[0.07]">
            ...
            {/* Mic button — always rendered, never disabled */}
            <div className="relative">
              {isListening && (
                <>
                  <span className="absolute inset-0 rounded-full bg-red-500/15 animate-ping" style={{ animationDuration: "1.5s" }} />
                  <span className="absolute -inset-4 rounded-full border border-red-500/10 animate-ping" style={{ animationDuration: "2s" }} />
                </>
              )}
              <button
            ...
            {micError && (
              <div className="w-full rounded-xl bg-red-500/10 border border-red-500/20 px-4 py-3 flex gap-3 animate-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="space-y-1.5">
                  <p className="text-xs text-red-300/90 leading-relaxed font-medium">Microphone access denied</p>
                  <p className="text-[11px] text-red-300/60 leading-relaxed">
                    Click the 🔒 lock icon in your browser address bar → find Microphone → set it to Allow → refresh the page.
                  </p>
                </div>
              </div>
            )}
            ...
            {results.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-white/[0.07] border-dashed py-12">
                ...
                <p className="text-xs text-white/20 text-center max-w-[160px] leading-relaxed">
                  {isListening
                    ? "Waiting for a Bible reference…"
                    : "Tap the mic and start the service"}
                </p>
              </div>
            ) : (
...
function VerseCard({ verse, active, onSend }: { verse: ResolvedVerse; active: boolean; onSend: () => void }) {
  const dot = verse.confidence === "high" ? "bg-emerald-400" : verse.confidence === "medium" ? "bg-amber-400" : "bg-red-400";
